...
- `runtime/execution_state.json`：当天执行时段和每个账号最后成功步数的唯一状态账本；按北京时间覆盖，不保留每日历史文件。
- `main.py`：刷步业务入口。
...
- 业务计划使用明确的北京时间；GitHub Actions cron 使用对应 UTC 时间，并在文档中同时标注北京时间，避免依赖时区调度。
- 每日执行窗口从北京时间 08:30 开始，每 2 小时一个窗口，至 20:30；主计划在窗口开始后随机延迟 0–60 分钟，补执行只能在该窗口结束后介入。
- 自动补执行必须有每日上限，且每次运行最多补 1 个缺失时段，只统计成功完成的刷步任务。
- 每个账号当天后续成功步数必须大于上一次成功步数；账号状态键不得写入明文账号。
