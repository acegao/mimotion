...
- CONFIG的内容：
  ...
  | MIN_STEP                | 最小步数，实际随机范围会随时间线性增加，并会自动高于同账号当天上一次成功步数                                                                        |
  | MAX_STEP                | 最大步数，最大步数和最小步数随机范围随着时间线性增加，北京时间22点达到最大值；如同账号当天上一次成功步数已高于当前时间上限，会优先保证本次步数递增 |
...
- 当天完成状态和每个账号最后成功步数保存在 `runtime/execution_state.json`；迟到的定时任务会先检查状态，不会重复补同一时段；等待中的巡检会保留完整队列，避免被后续巡检替换。
...
8. 最大步数和最小步数随着时间增长，10点执行时基础范围为10/22 \* 18000 ~ 10/22 \* 25000：8181 ~
   11363，以此类推，在北京时间22点达到最大值。脚本会记录同账号当天上一次成功步数，后续执行会从 `上一次成功步数 + 1` 起随机，保证当天成功步数递增。要修改基础范围可以修改CONFIG中的MIN_STEP和MAX_STEP。
...
- `runtime/execution_state.json` 记录北京时间当天已成功完成的计划时段和每个账号最后成功步数；日期变化后会自动重置。
