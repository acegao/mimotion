...
当前计划按北京时间每天执行 7 次：`08:30、09:30、11:30、13:30、15:30、17:30、19:30`。

- 主计划配置在 `.github/workflows/run.yml`，cron 使用 UTC 等价值 `30 0,1,3,5,7,9,11 * * *`，对应上述北京时间。
- `08:07` 至 `22:52` 每 15 分钟巡检一次；发现当天应执行但尚未完成的时段，每次最多补执行 1 个。
...
1. 默认每天完成 7 个计划时段。主计划触发后会随机延迟 0–60 分钟；巡检任务不随机延迟，只负责补齐已到期但未完成的时段，且单次最多补 1 次，避免集中刷。
