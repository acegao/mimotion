...
- `09:17` 至 `22:17` 每小时巡检一次；发现当天应执行但尚未完成的时段，会串行补执行。
- 当天完成状态保存在 `runtime/execution_state.json`，迟到的定时任务会先检查状态，不会重复补同一时段。
...
1. 默认每天完成 7 个计划时段。主计划触发后会随机延迟 0–60 分钟；巡检任务不随机延迟，只负责补齐已到期但未完成的时段。
