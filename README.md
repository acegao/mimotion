...
- CONFIG的内容：
  ...
  | MAX_STEP                | 最大步数，最大步数和最小步数按当天时间曲线平滑增加；如同账号当天上一次成功步数已高于当前时间上限，会优先保证本次步数递增                         |
...
8. 最大步数和最小步数按当天时间曲线平滑增加，08:30 附近约为 MIN_STEP/MAX_STEP 的 16%，21:30 附近约为 95%。以默认 `MIN_STEP=18000`、`MAX_STEP=25000` 计算，08:30 基础范围约为 `2880~4000`，之后随时间逐步抬升。脚本会记录同账号当天上一次成功步数，后续执行会从 `上一次成功步数 + 1` 起随机，保证当天成功步数递增；第二天重新从低起点开始。要修改基础范围可以修改CONFIG中的MIN_STEP和MAX_STEP。
