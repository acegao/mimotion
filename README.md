...
- CONFIG的内容：
  ...
  | MIN_STEP                | 最小步数，实际随机范围按当天时间曲线平滑增加，并会自动高于同账号当天上一次成功步数                                                                |
  | MAX_STEP                | 最大步数，最大步数和最小步数按当天时间曲线平滑增加；实际随机上限会控制在 18000 以内；如同账号当天上一次成功步数已高于当前时间上限，会优先保证本次步数递增 |
...
当前计划按北京时间每天执行 7 次，执行窗口从 `08:30` 开始，每 2 小时一次：`08:30、10:30、12:30、14:30、16:30、18:30、20:30`。

- 主计划配置在 `.github/workflows/run.yml`，cron 使用 UTC 等价值 `30 0,2,4,6,8,10,12 * * *`，对应上述北京时间窗口开始时间；每次随机延迟 0–60 分钟后执行。
- 每个窗口开始 75 分钟后检查一次是否漏跑，对应北京时间 `09:45、11:45、13:45、15:45、17:45、19:45、21:45`；每次最多补执行 1 个。
- 当天完成状态和每个账号最后成功步数保存在 `runtime/execution_state.json`；日期变化会重置当天完成时段和最后成功步数；迟到的定时任务会先检查状态，不会重复补同一时段；等待中的巡检会保留完整队列，避免被后续巡检替换。
- `CRON_HOURS` 和 `Random Cron` 已停用，不再决定执行时间。
- 如需调整计划时间，必须同时修改 `run.yml` 中的主计划 cron 和 `execution_reconciler.py` 中的 `SLOTS`。
...
1. 默认每天完成 7 个计划时段，从北京时间 08:30 起约每 2 小时一次。主计划触发后会随机延迟 0–60 分钟，因此首次执行不会早于 08:30；补执行检查不随机延迟，只负责补齐未完成的时段，且单次最多补 1 次，避免集中刷。

2. 多账户的数量和密码请一定要对上 不然无法使用!!!

3. 当前工作流的 cron 使用 UTC 时间；北京时间固定为 UTC+8，修改计划时必须先换算为 UTC，并同步更新文档和 `execution_reconciler.py`。
...
8. 最大步数和最小步数按当天时间曲线平滑增加，08:30 附近约为 MIN_STEP/MAX_STEP 的 18%，21:30 附近约为 65%，且实际随机上限会控制在 `18000` 以内。以默认 `MIN_STEP=18000`、`MAX_STEP=25000` 计算，09:30 基础范围约为 `3400~4700`，18:30 基础范围约为 `10500~14600`，全天最高约 `11700~16250`。脚本会记录同账号当天上一次成功步数，后续执行会从 `上一次成功步数 + 1` 起随机，保证当天成功步数递增；第二天重新从低起点开始。要修改基础范围可以修改CONFIG中的MIN_STEP和MAX_STEP。
...
- 点击其中一条记录，可以查看执行详情，这里以 `刷步数` 为例：
    ...
    - 执行步骤中主要关注 `检查并补齐今日执行`，点击后可查看本次补了哪些计划时段
    - 展开后便可以查看到执行日志，如果执行成功，则会显示每个账号当前随机的步数是多少
    - 如果执行失败，则需要根据实际情况分析具体失败原因
- `runtime/execution_state.json` 记录北京时间当天已成功完成的计划时段和每个账号最后成功步数；日期变化后会重置当天计划时段和最后成功步数，让每天重新从较低步数开始。
